...
- **HTTP 422 on `run-with-report` with `curl -F payload=@payload.json`:** `curl` sends `payload` as a **file part** (with a filename). Older FastAPI `Form(str)` could not accept that and returned 422. The API now reads multipart manually so **`payload` may be a plain string or an uploaded file**—either works after deploy.
